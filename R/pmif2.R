...
pmif2.internal <- function (object, Nmcmc,
                            start, pars, ivps,
                            particles,
  ...
  if (is.null(start.names))
    stop("pmif2 error: ",sQuote("start")," must be a named vector",call.=FALSE)

  if (missing(rw.sd))
    stop("pmif2 error: ",sQuote("rw.sd")," must be specified",call.=FALSE)
  rw.names <- names(rw.sd)
  rwsdMat <-rw.sd
  if (is.null(rw.names) || any(rw.sd<0))
    stop("pmif2 error: ",sQuote("rw.sd")," must be a named non-negative numerical vector",call.=FALSE)
  if (!all(rw.names%in%start.names))
    stop("pmif2 error: all the names of ",sQuote("rw.sd")," must be names of ",sQuote("start"),call.=FALSE)
  rw.names <- names(rw.sd[rw.sd>0])
  if (length(rw.names) == 0)
    stop("pmif2 error: ",sQuote("rw.sd")," must have one positive entry for each parameter to be estimated",call.=FALSE)

  if (missing(pars))
    stop("pmif2 error: ",sQuote("pars")," must be specified",call.=FALSE)
  if (length(pars)==0)
    stop("pmif2 error: at least one parameter must be estimated",call.=FALSE)
  if (
      !is.character(pars) ||
      !is.character(ivps) ||
  ...
  if (missing(Nacceptance))
    Nacceptance=0
  if (missing(Nmcmc))
    stop("pmif2 error: ",sQuote("Nmcmc")," must be specified",call.=FALSE)
  Nmcmc <- as.integer(Nmcmc)
  if (Nmcmc<0)
    stop("pmif2 error: ",sQuote("Nmcmc")," must be a positive integer",call.=FALSE)
    if ((method=="mif2")&&(Np[1L]!=Np[ntimes+1]))
    stop("the first and last values of ",sQuote("Np")," must agree when method = ",sQuote("mif2"))
  
  if (verbose) {
    cat("performing",Nmcmc,"pmif2 iteration(s) to estimate parameter(s)",
        paste(pars,collapse=", "))
 ...
 if (Nmcmc>0) {
    tmp.pmif2 <- new("pmif2",object,particles=particles,Np=Np[1L])
  } else {
    pfp <- obj
  }
  ...
  for (n in seq_len(Nmcmc)) { # main loop
    ...
    if (inherits(pfp.prop,'try-error'))
      stop("pmif2 error: error in ",sQuote("pfilter2"),call.=FALSE)
    log.prior.prop <- dprior(object,params=theta.prop,log=TRUE,.getnativesymbolinfo=gnsi)
    gnsi <- FALSE

    ## pmif2 update rule (OK because proposal is symmetric)
    ...
    if (verbose) cat("pmif2 iteration ",n," of ",Nmcmc," completed\n")
...
setMethod(
          ...
          function (object, Nmcmc = 1,
                    ...
                    method = c("pmif2","punweighted","pfp","pmif2","pmif2","pmif2"),
                    lag=0, 
  	    ...
  	    if (missing(rw.sd))
            ...
            if (missing(Np))
              stop("pmif2 error: ",sQuote("Np")," must be specified",call.=FALSE)
